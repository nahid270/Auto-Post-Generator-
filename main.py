...
import random # ✨ ওয়াটারমার্কের রঙের জন্য নতুন ইম্পোর্ট

# --- Third-party Library Imports ---
from PIL import Image, ImageDraw, ImageFont, ImageEnhance # ✨ ওয়াটারমার্কের জন্য নতুন ইম্পোর্ট
...
# ---- ✨ নতুন এবং উন্নত ওয়াটারমার্ক ফাংশন ✨ ----
def watermark_poster(poster_url: str, watermark_text: str):
    ...
    try:
        ...
        if watermark_text:
            ...
            # ফন্টের সাইজ ছবির প্রস্থের উপর ভিত্তি করে ডাইনামিকভাবে সেট করা
            font_size = int(img.width / 12)
            try:
                font = ImageFont.truetype("Poppins-Bold.ttf", font_size)
            except IOError:
                font = ImageFont.truetype("arial.ttf", font_size) # ফলব্যাক ফন্ট

            # পোস্টার থেকে একটি প্রভাবশালী রঙ বের করা
            ...
            # টেক্সটের জন্য একটি কনট্রাস্টিং রঙ তৈরি করা
            ...
            y = img.height - text_height - (img.height * 0.05) # নিচ থেকে ৫% উপরে

            # ব্ল্যাক শ্যাডো দিয়ে টেক্সটকে আরও স্পষ্ট করা
            draw.text((x + 2, y + 2), watermark_text, font=font, fill=(0, 0, 0, 128))
            # মূল ওয়াটারমার্ক টেক্সট
...
# ---- ✨ নতুন এবং উন্নত ক্যাপশন ফাংশন (বড় ডাউনলোড লিংক) ✨ ----
def generate_channel_caption(data: dict, language: str, links: dict):
    ...
    # ---- মুভির জন্য টেমপ্লেট ----
        ...
        caption = (f"🎬 **{title} ({year})**\n\n"
                   ...
                   "📥 **ডাউনলোড লিংকসমূহ** 📥\n"
                   "▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬\n")
        if links.get("480p"): caption += f"✅ **[480p কোয়ালিটি ডাউনলোড]({links['480p']})**\n"
        if links.get("720p"): caption += f"✅ **[720p কোয়ালিটি ডাউনলোড]({links['720p']})**\n"
        if links.get("1080p"): caption += f"✅ **[1080p কোয়ালিটি ডাউনলোড]({links['1080p']})**\n"
...
async def conversation_handler(client, message: Message):
    ...
    if state == "wait_movie_lang":
        ...
        if text.lower() == 'done':
            ...
            await generate_final_post_preview(client, uid, message.chat.id, msg)
            return
        if not text.isdigit() or int(text) <= 0: return await message.reply_text("❌ ভুল নম্বর। দয়া করে একটি সঠিক সিজন নম্বর লিখুন (যেমন: 1)।")
...
async def post_to_channel_cb(client, cb: Message):
    ...
    convo = user_conversations.get(uid)
    channel_data = db_query("SELECT channel_id FROM users WHERE user_id=?", (uid,), 'one')
    if not convo or not channel_data or not channel_data[0]: return await cb.message.edit("❌ সেশন বা চ্যানেল আইডি পাওয়া যায়নি।")
    ...
    except Exception as e: await cb.message.edit(f"❌ চ্যানেলে পোস্ট করতে সমস্যা হয়েছে: {e}")
    finally:
...
async def test_post_command(client, message: Message):
    ...
    channel_data = db_query("SELECT channel_id FROM users WHERE user_id=?", (uid,), 'one')
    if not channel_data or not channel_data[0]: return await processing_msg.edit("❌ আপনার কোনো চ্যানেল সেট করা নেই।")
    ...
    try:
        await client.send_message(chat_id=channel_id, text="✅ এটি বট থেকে পাঠানো একটি পরীক্ষামূলক বার্তা।")
        await processing_msg.edit(f"✅ সফলভাবে `{channel_id}` চ্যানেলে পরীক্ষামূলক বার্তা পাঠানো হয়েছে!")
    except Exception as e: await processing_msg.edit(f"❌ চ্যানেলে বার্তা পাঠাতে ব্যর্থ!\n\n**ত্রুটি:**\n`{e}`")
